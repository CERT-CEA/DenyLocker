...
else
{
    ...
    if ($AppLockerXML.Length -gt 0)
    {
        ...
        $xlFname = [System.IO.Path]::GetFileNameWithoutExtension($AppLockerXML)
        if ($SaveWorkbook)
        {
    ...
    else
    {
        ...
        if ($Local)
        {
            ...
            $xlFname = "applocker_local_policy"
        }
        else
        {
            $tabname = "AppLocker policy - Effective"
            $xlFname = "applocker_effective_policy"
        }
    }
}
